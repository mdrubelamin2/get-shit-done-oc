...
**Core principle:** Antigravity automates everything with CLI/API. Checkpoints are for verification and decisions, not manual work.

**Golden rules:**
1. **If Antigravity can run it, Antigravity runs it** - Never ask user to execute CLI commands, start servers, or run builds
2. **Antigravity sets up the verification environment** - Start dev servers, seed databases, configure env vars
3. **User only does what requires human judgment** - Visual checks, UX evaluation, "does this feel right?"
4. **Secrets come from user, automation comes from Antigravity** - Ask for API keys, then Antigravity uses them via CLI
</overview>
...
**When:** Antigravity completed automated work, human confirms it works correctly.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What Antigravity automated and deployed/built]</what-built>
  <how-to-verify>
...
**Key elements:**
- `<what-built>`: What Antigravity automated (deployed, built, configured)
...
**Key pattern:** Antigravity starts the dev server BEFORE the checkpoint. User only needs to visit the URL.
...
**When:** Action has NO CLI/API and requires human-only interaction, OR Antigravity hit an authentication gate during automation.

**Use ONLY for:**
- **Authentication gates** - Antigravity tried to use CLI/API but needs credentials to continue (this is NOT a failure)
...
<task type="checkpoint:human-action" gate="blocking">
  <action>[What human must do - Antigravity already did everything automatable]</action>
  <instructions>
    [What Antigravity already automated]
    [The ONE thing requiring human action]
  </instructions>
  <verification>[What Antigravity can check afterward]</verification>
  <resume-signal>[How to continue]</resume-signal>
</task>
```

**Key principle:** Antigravity automates EVERYTHING possible first, only asks human for the truly unavoidable manual step.
...
<!-- If vercel returns "Error: Not authenticated", Antigravity creates checkpoint on the fly -->
...
<!-- After authentication, Antigravity retries the deployment -->
...
**Key distinction:** Authentication gates are created dynamically when Antigravity encounters auth errors during automation. They're NOT pre-planned - Antigravity tries to automate first, only asks for credentials when blocked.
</type>
</checkpoint_types>

<execution_protocol>

When Antigravity encounters `type="checkpoint:*"`:
...
**Critical:** When Antigravity tries CLI/API and gets auth error, this is NOT a failure - it's a gate requiring human input to unblock automation.

**Pattern:** Antigravity tries automation → auth error → creates checkpoint → you authenticate → Antigravity retries → continues
...
```
Antigravity: Running `vercel --yes` to deploy...
...
Antigravity: Verifying authentication...
...
**Key distinction:**
- Pre-planned checkpoint: "I need you to do X" (wrong - Antigravity should automate)
...
**The rule:** If it has CLI/API, Antigravity does it. Never ask human to perform automatable work.
...
<!-- RIGHT: Antigravity asks for value, then adds via CLI -->
<task type="checkpoint:human-action">
...
**Antigravity starts servers, user visits URLs:**
...
**Pattern:**
```xml
<!-- Antigravity starts server before checkpoint -->
<task type="auto">
...
| Action | Automatable? | Antigravity does it? |
|--------|--------------|-----------------|
...
**DON'T:**
- Ask human to do work Antigravity can automate (deploy, create resources, run builds)
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" ❌ (too vague)
- Mix multiple verifications in one checkpoint (split them)
- Make verification impossible (Antigravity can't check visual appearance without user confirmation)

**Placement:**
- **After automation completes** - not before Antigravity does the work
- **After UI buildout** - before declaring phase complete
- **Before dependent work** - decisions before implementation
- **At integration points** - after configuring external services

**Bad placement:**
- Before Antigravity automates (asking human to do automatable work) ❌
...
```xml
<!-- Antigravity automates everything -->
<task type="auto">
  <name>Deploy to Vercel</name>
...
<!-- NO CHECKPOINT NEEDED - Antigravity automated everything and verified programmatically -->
```
...
<!-- ONE checkpoint at end verifies the complete flow - Antigravity already started server -->
<task type="checkpoint:human-verify" gate="blocking">
...
**Why bad:** Antigravity can run `npm run dev`. User should only visit URLs, not execute commands.

### ✅ GOOD: Antigravity starts server, user visits
...
**Why bad:** Convex has `npx convex env set`. Antigravity should ask for the key value, then run the CLI command.

### ✅ GOOD: Antigravity collects secret, adds via CLI
...
### ✅ GOOD: Antigravity automates, human verifies
...
**Why bad:** Antigravity has Write tool. This should be `type="auto"`.
...
**Why bad:** Antigravity can run these commands. User should never execute CLI commands.
...
**Why bad:** Stripe has an API. Antigravity should create the webhook via API and write to .env directly.

</anti_patterns>

<summary>

Checkpoints formalize human-in-the-loop points. Use them when Antigravity cannot complete a task autonomously OR when human verification is required for correctness.

**The golden rule:** If Antigravity CAN automate it, Antigravity MUST automate it.

**Checkpoint priority:**
1. **checkpoint:human-verify** (90% of checkpoints) - Antigravity automated everything, human confirms visual/functional correctness
...
- Things Antigravity can verify programmatically (tests pass, build succeeds)
- File operations (Antigravity can read files to verify)
