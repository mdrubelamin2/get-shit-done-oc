...
You are planning for ONE person (the user) and ONE implementer (Antigravity).
- No teams, stakeholders, ceremonies, coordination overhead
- User is the visionary/product owner
- Antigravity is the builder
- Estimate effort in Antigravity execution time, not human dev time
...
Antigravity degrades when it perceives context pressure and enters "completion mode."

| Context Usage | Quality | Antigravity's State |
|---------------|---------|----------------|
...
|------|---------|----------|
| `auto` | Everything Antigravity can do independently | Fully autonomous |
| `checkpoint:human-verify` | Visual/functional verification | Pauses for user |
| `checkpoint:decision` | Implementation choices | Pauses for user |
| `checkpoint:human-action` | Truly unavoidable manual steps (rare) | Pauses for user |

**Automation-first rule:** If Antigravity CAN do it via CLI/API, Antigravity MUST do it. Checkpoints are for verification AFTER automation, not for manual work.

## Task Sizing

Each task should take Antigravity **15-60 minutes** to execute. This calibrates granularity:
...
**The test:** Could a different Antigravity instance execute this task without asking clarifying questions? If not, add specificity.
...
Record in `user_setup` frontmatter. Only include what Antigravity literally cannot do (account creation, secret retrieval, dashboard config).
...
<execution_context>
.agent/resources/workflows/execute-plan.md
.agent/resources/templates/summary.md
</execution_context>
...
Only include what Antigravity literally cannot do (account creation, secret retrieval, dashboard config).
...
Human confirms Antigravity's automated work works correctly.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What Antigravity automated]</what-built>
  <how-to-verify>
...
When Antigravity tries CLI/API and gets auth error, this is NOT a failure - it's a gate.

Pattern: Antigravity tries automation -> auth error -> creates checkpoint -> user authenticates -> Antigravity retries -> continues

Authentication gates are created dynamically when Antigravity encounters auth errors during automation. They're NOT pre-planned.
...
**DON'T:**
- Ask human to do work Antigravity can automate
